...
*/
//:: Updated 03/19/2023 - reset phenotype to normal if using one of the ACP phenotypes
//:: Updated 03/25/2023 - cancel levitation if we're using that
//::                    - half-ogres can't ride mounts
...
void main()
{
   ...
   //* Pstemarie - reset ACP phenotype to normal or you'll get "horse hat"
   ...
   { // SPELL/FEAT ID - switch to control horse actions
       ...
       { // Mount Menu Selection
           //* Pstemarie - remove levitation
           RemoveEffectsFromSpell(oPC, 871);
           //* remove levitation
           if (GetRacialType(oPC) == 41)
           { //* Pstemarie - half-ogres are too big to ride horses
               FloatingTextStringOnCreature(StringToRGBString("You're too big to ride a horse!",STRING_COLOR_PINK),oPC,FALSE);
               return;
           } //* Pstemarie - half-ogres are too big to ride horses
           ...
           { // mountable
               ...
               { // PC Can mount it
                   ...
                   { // saddlebags
                       ...
                       ActionDoCommand(HorseMount(oTarget,TRUE));//1.72: fix for being able to mount horse in inaccessable area
                   } // saddlebags
                   else
                   { // not saddlebags
                       ClearAllActions();
       ...
       { // Party Mount Menu Selection
           ...
           { // PC mount this target
               ...
               ClearAllActions();
               ActionMoveToObject(oTarget);
               ActionDoCommand(HorseMount(oTarget,FALSE));//1.72: fix for being able to mount horse in inaccessable area
           } // PC mount this target
           nN=1;
           ...
           { // check to see which henchmen can mount and if possible make them mount
               ...
               { // try to mount
                   if (GetRacialType(oHench) == 41)
                   { //* Pstemarie - half-ogres are too big to ride horses
                       FloatingTextStringOnCreature(StringToRGBString("I'm too big to ride a horse!",STRING_COLOR_PINK),oHench,FALSE);
                       return;
                   } //* Pstemarie - half-ogres are too big to ride horses
                   //* Pstemarie - remove levitation
                   RemoveEffectsFromSpell(oHench, 871);
                   //* remove levitation
...
{ // PURPOSE: Summon the Paladin mount and mount it
    ...
    {
        ClearAllActions();
        ActionMoveToObject(oHorse);
        ActionDoCommand(HorseMount(oHorse,bAnimate));//1.72: fix for being able to mount horse in inaccessable area
    }
...
{ // PURPOSE: Handle Mounting for this henchman
    ...
    { // Valid target
        ...
        AssignCommand(oHenchman,ActionMoveToObject(oHorse));
        AssignCommand(oHenchman,ActionDoCommand(HorseMount(oHorse,FALSE)));//1.72: fix for being able to mount horse in inaccessable area
    } // valid target
    else
    { // not valid
      // check for paladin mount
